...
@app.post("/webhooks/typeform/lead-magnet")
async def typeform_lead_magnet_webhook(payload: dict, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """Handle Typeform webhook for lead magnet downloads"""
    try:
        # Extract email from Typeform payload
        ...
        if not email:
            logger.error("No email found in Typeform lead magnet webhook payload")
            return {"success": False, "error": "Email not found in payload"}
        
        # Create lead magnet request
        request = LeadMagnetRequest(email=email)
        
        # Use existing lead magnet endpoint logic
        return await download_guide(request, background_tasks, db)
        
    except Exception as e:
        logger.error(f"Error processing Typeform lead magnet webhook: {str(e)}")
        return {"success": False, "error": str(e)}

@app.post("/webhooks/typeform/corporate")
async def typeform_corporate_webhook(payload: dict, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """Handle Typeform webhook for corporate inquiries"""
    try:
        # Extract form data from Typeform payload
        answers = payload.get("form_response", {}).get("answers", [])
        form_data = {}
        
        # Map Typeform answers to expected fields
        field_mapping = {
        ...
        for answer in answers:
            ...
            if field_ref in field_mapping:
                if answer_type == "email":
                    form_data[field_mapping[field_ref]] = answer.get("email")
                elif answer_type == "short_text" or answer_type == "long_text":
                    form_data[field_mapping[field_ref]] = answer.get("text")
                elif answer_type == "phone_number":
                    form_data[field_mapping[field_ref]] = answer.get("phone_number")
        
        # Validate required fields
        required_fields = ["companyName", "contactPerson", "email", "trainingGoals"]
        missing_fields = [field for field in required_fields if not form_data.get(field)]
        
        if missing_fields:
            return {"success": False, "error": f"Missing required fields: {missing_fields}"}
        
        # Create corporate inquiry request
        request = CorporateInquiryRequest(**form_data)
        
        # Use existing corporate inquiry endpoint logic
        return await submit_corporate_inquiry(request, background_tasks, db)
        
    except Exception as e:
        logger.error(f"Error processing Typeform corporate webhook: {str(e)}")
        return {"success": False, "error": str(e)}

@app.post("/webhooks/typeform/waitlist")
async def typeform_waitlist_webhook(payload: dict, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """Handle Typeform webhook for waitlist registrations"""
    try:
        # Extract form data from Typeform payload
        answers = payload.get("form_response", {}).get("answers", [])
        form_data = {}
        ...
        # Validate required fields
        required_fields = ["full_name", "email", "city_country", "occupation", "why_join", "skills_to_improve"]
        missing_fields = [field for field in required_fields if not form_data.get(field)]
        
        if missing_fields:
            return {"success": False, "error": f"Missing required fields: {missing_fields}"}
        
        # Create waitlist registration request
        request = WaitlistRegistrationRequest(**form_data)
        
        # Use existing waitlist registration endpoint logic
        return await register_waitlist(request, background_tasks, db)
        
    except Exception as e:
        logger.error(f"Error processing Typeform waitlist webhook: {str(e)}")
        return {"success": False, "error": str(e)}
